...
def standard_scaler(data, indices):
    ...
    for i in indices:
        ...
        transformed_image = transformed_image[:217, :217]
...
def standard_scaler_with_train(data, indices, mean=None, std=None):
    ...
    -------
    scaled_data: numpy
        scaled numpy time series dataset, of shape: time, width, height (e.g.: [10, 217, 217] for sequence of 10 images)
    mean, std: int
        mean and std of the scaled dataset
    """
...
def custom_transform(image):
    ...
    Transformation:
     ...
     - Crop to upper 217x217 pixels to isolate dynamic region of the Gulf Stream.
...
def inverse_scale(image, mean, std):
    ...
    ----
    image: tensor
        the predicted image as a tensor of shape torch.Size([217, 217])
    mean, std: tensor
