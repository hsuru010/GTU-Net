...
class time_series_dataset(Dataset):
    ...
    def __init__(self, datasets, L_source, L_target, dt, transform=None, model_is_3dconv=False, include_sst=False):
        ...
        if self.include_sst:
            self.dataset_adt = datasets[0]
            self.dataset_sst = datasets[1]
        else:
            self.dataset_adt = datasets
    ...
    def __len__(self):
        return len(self.dataset_adt) - self.L_source - self.L_target + 1
    ...
    def __getitem__(self, idx):
        ...
        if self.include_sst:
            item = {}
            item['input_adt'], item['input_sst'] = self.dataset_adt[input_start:input_end], self.dataset_sst[input_start:input_end]
            item['target_adt'], item['target_sst'] = self.dataset_adt[input_end:output_end], self.dataset_sst[input_end:output_end]
        else:
            input_images = self.dataset_adt[input_start:input_end]
            target_images = self.dataset_adt[input_end:output_end]
        
        if self.transform:
            if self.include_sst:
                item['input_adt'] = torch.stack([self.transform(image) for image in item['input_adt']])
                item['input_sst'] = torch.stack([self.transform(image) for image in item['input_sst']])
                item['target_adt'] = torch.stack([self.transform(image) for image in item['target_adt']])
        ...
        if self.model_is_3dconv is True:
            if self.include_sst:
                item['input_adt'], item['input_sst'] = item['input_adt'].permute(1, 0, 2, 3), item['input_sst'].permute(1, 0, 2, 3)
                item['target_adt'], item['target_sst'] = item['target_adt'].permute(1, 0, 2, 3), item['target_sst'].permute(1, 0, 2, 3)
            else:
        ...
        # else, reshape data to flatten the time channel and sequence dimensions into one dimension for 2d convolutions
            ...
            if self.include_sst:
                item['input_adt'], item['input_sst'] = item['input_adt'].view(-1, 120, 160), item['input_sst'].view(-1, 120, 160)
                item['target_adt'], item['target_sst'] = item['target_adt'].view(-1, 120, 160), item['target_sst'].view(-1, 120, 160)
            else:
                input_images = input_images.view(-1, 120, 160)
                target_images = target_images.view(-1, 120, 160)
...
class time_series_module(L.LightningDataModule):
    ...
    def setup(self):
        ...
        if self.dataloader_config['scale_with_train']:
            if self.dataloader_config['include_sst']:
                scaled_train_set_adt, train_mean_adt, train_std_adt = standard_scaler_with_train(train_set[0], list(range(len(train_set[0]))))
                scaled_val_set_adt, _, _ = standard_scaler_with_train(val_set[0], list(range(len(val_set[0]))), 
                                                                      mean=train_mean_adt, std=train_std_adt)
                scaled_test_set_adt, self.mean_adt, self.std_adt = standard_scaler_with_train(test_set[0], list(range(len(test_set[0]))), 
                                                                                              mean=train_mean_adt, std=train_std_adt)
                ...
                scaled_test_set_sst, self.mean_sst, self.std_sst = standard_scaler_with_train(test_set[1], list(range(len(test_set[1]))), 
                                                                                              mean=train_mean_sst, std=train_std_sst)
                scaled_train_set = (scaled_train_set_adt, scaled_train_set_sst)
                scaled_val_set = (scaled_val_set_adt, scaled_val_set_sst)
                scaled_test_set = (scaled_test_set_adt, scaled_test_set_sst)
                self.mean = (self.mean_adt, self.mean_sst)
                self.std = (self.std_adt, self.std_sst)
            else:
            ...
            if self.dataloader_config['include_sst']:
                scaled_train_set_adt, train_mean_adt, train_std_adt = standard_scaler(train_set[0], list(range(len(train_set[0]))))
                scaled_val_set_adt, _, _ = standard_scaler(val_set[0], list(range(len(val_set[0]))))
                scaled_test_set_adt, self.mean_adt, self.std_adt = standard_scaler(test_set[0], list(range(len(test_set[0]))))
    ...
    def train_dataloader(self):
        training_generator = DataLoader(self.training_set,
                                        num_workers=5, # to accelerate training
