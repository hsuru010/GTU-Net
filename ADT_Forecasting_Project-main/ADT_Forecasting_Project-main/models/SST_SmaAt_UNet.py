...
class SmaAt_UNet_SST(nn.Module):
    def __init__(self, n_channels, n_classes, kernels_per_layer=2, bilinear=True, reduction_ratio=16):
        ...
        # Modify the decoder structure to take inputs from the corresponding level of the encoder and the ADT branch
    ...
    def forward(self, x_adt, x_sst):
        ...
        x1_adt = self.inc(x_adt)
        x1_adt_att = self.cbam1(x1_adt)
        x1_sst = self.inc_sst(x_sst)
        x1_sst_att = self.cbam1_sst(x1_sst)
        x1 = torch.cat([x1_adt_att, x1_sst_att], dim=1)
        x2 = self.down1(x1)
