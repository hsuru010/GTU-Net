...
class TAU(nn.Module):
    def __init__(self, input_channels, hidden_dim=64, kernel_size=3):
        ...
        self.conv_lstm = nn.LSTM(input_size=input_channels, hidden_size=hidden_dim, num_layers=1, batch_first=True)
        self.conv = nn.Conv2d(hidden_dim, input_channels, kernel_size=kernel_size, padding=kernel_size // 2)

    def forward(self, x):
        batch, channels, height, width = x.size()
        x = x.view(batch, channels, -1).permute(0, 2, 1)  # Reshape to (batch, seq_len, channels)
        x, _ = self.conv_lstm(x)  # Apply LSTM to capture temporal dependencies
        x = x.permute(0, 2, 1).view(batch, -1, height, width)  # Reshape back to (batch, channels, H, W)
        x = self.conv(x)  # Apply convolution for spatial refinement
