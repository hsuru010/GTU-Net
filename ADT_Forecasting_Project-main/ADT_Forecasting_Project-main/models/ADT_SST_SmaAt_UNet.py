...
class SmaAt_UNet_ADT_SST(nn.Module):
    ...
    def forward(self, x_adt, x_sst):

        
        last_adt_pos = x_adt.shape[1]//2 - 1
        last_adt = x_adt[:, last_adt_pos, :, :]
        last_adt_input=torch.unsqueeze(last_adt, axis=1)
        
        last_sst = x_sst[:, -1, :, :]
        last_sst_input=torch.unsqueeze(last_sst, axis=1)
            
        # concatenate the adt and SST sequences along the channel dimension
        x = torch.cat([x_adt, x_sst], dim=1)
        ...
        # split the output sequence into adt and SST
        adt_pred, sst_pred = torch.chunk(logits, 2, dim=1)

        # add last adt and sst timesteps and return
        return adt_pred + last_adt_input, sst_pred + last_sst_input